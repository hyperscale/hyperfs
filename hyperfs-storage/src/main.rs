...
async fn main() -> Result<(), Box<dyn core::error::Error>> {
    ...
    let bind_addr: SocketAddr = cli.advertise_addr.parse().expect("Invalid address");
    ...
    let announce_to: Option<ID> = {
        if let Some(value) = cli.announce_to {
            ...
            while let Some(notification) = runtime.to_notify() {
                match notification {
                    OwnedNotification::MemberUp(_) | OwnedNotification::MemberDown(_) => {
                        ...
                        tracing::info!(
                            members = tracing::field::debug(foca.iter_members().count()),
                            "cluster changed"
                        );
                    }
